...
function renderPage() {
  ...
  renderStreetviewBarChart();
  bindStreetviewMetricSwitch();
...
const FEATURE_LABELS = {
  ...
  sky_openness: "天空开敞度",
  brightness_mean: "街景亮度",
  green_channel_mean: "绿色通道强度",
  entropy_mean: "街景丰富度",
  edge_density_mean: "街景边缘密度",
};
...
const SCENARIO_LABELS = {
  ...
  C: "补充餐饮服务",
  D: "优化路网连接",
  A_subway_zero: "新增近距离地铁入口",
  B_pedestrian_150pct: "补强步行通道",
  C_food_to_q75: "补充餐饮服务",
  D_intersection_to_q80: "优化路网连接",
  E_edge_density_to_q90: "提升街景边缘密度",
};
...
function publicScenarioLabel(value) {
  ...
  if (normalized.includes("edge")) return "提升街景边缘密度";
...
function renderBarChart(svg, items, opts) {
  clear(svg);
  const options = opts || {};
  const maxBars = options.maxBars || items.length;
  const displayItems = items.slice(0, maxBars);
  const width = 720;
  const height = Math.max(320, displayItems.length * 18 + 36);
  svg.setAttribute("viewBox", `0 0 ${width} ${height}`);
  const margin = { top: 18, right: 22, bottom: 18, left: 120 };
  const innerWidth = width - margin.left - margin.right;
  const innerHeight = height - margin.top - margin.bottom;
  const maxValue = Math.max(...displayItems.map((item) => Number(item.value)), 1);
  const barHeight = innerHeight / Math.max(displayItems.length, 1);
  const group = svgEl("g", { transform: `translate(${margin.left},${margin.top})` });
  svg.append(group);
...
function renderStreetMetrics(summary) {
  ...
  clear(grid);
  const display = (value, suffix = "") => (value === null || value === undefined ? "暂无" : `${Number(value).toFixed(3)}${suffix}`);
  const items = [
    ["边缘密度", display(summary.edge_density_mean)],
    ["信息熵", display(summary.entropy_mean)],
    ["绿视率", display(summary.green_view_index)],
    ["天空开阔度", display(summary.sky_openness)],
    ["亮度", display(summary.brightness_mean)],
    ["绿色通道", display(summary.green_channel_mean)],
    ["覆盖网格", `${summary.sampleCount || 0} / 105`],
    ["覆盖率", `${(Number(summary.coverageRate || 0) * 100).toFixed(0)}%`],
  ];
...
function renderStreetviewBarChart() {
  const svg = document.getElementById("streetviewBarChart");
  if (!svg) return;
  const features = state.dashboard?.streetScene?.gridFeatures;
  if (!features || !features.length) return;
  const metricRadio = document.querySelector('input[name="svMetric"]:checked');
  const metric = metricRadio ? metricRadio.value : "edge_density_mean";
  const labelMap = {
    edge_density_mean: "边缘密度",
    entropy_mean: "信息熵",
    green_view_index: "绿视率",
    sky_openness: "天空开阔度",
    brightness_mean: "亮度",
  ...
  const items = sorted.map((f) => ({
    label: f.grid_id,
    value: f[metric],
    display: Number(f[metric]).toFixed(3),
    color: f.zone === "A" ? colors.jade : f.zone === "B" ? colors.amber : f.zone === "C" ? colors.terracotta : colors.muted,
  }));
  renderBarChart(svg, items, { title: labelMap[metric] || metric, maxBars: 40 });
}

function bindStreetviewMetricSwitch() {
  document.querySelectorAll('input[name="svMetric"]').forEach((radio) => {
    radio.addEventListener("change", () => renderStreetviewBarChart());
  });
}
...
function renderForecastHighlights() {
  ...
  clear(container);
  const highlights = state.dashboard.forecast.highlights || [];
...
function renderShapSection() {
  const shap = state.dashboard.shap;
  if (!shap || !shap.available) return;

  const cardsContainer = document.getElementById("shapSummaryCards");
  if (cardsContainer) {
    clear(cardsContainer);
    (shap.summaryCards || []).forEach((card) => {
      const chip = create("article", "camera-chip");
      chip.append(
      ...
      cardsContainer.append(chip);
    });
  }

  const featureSvg = document.getElementById("shapFeatureChart");
  if (featureSvg && shap.allFeatures) {
    const items = shap.allFeatures.map((f) => ({
      label: f.label,
      value: f.importance,
      display: f.importance.toFixed(4),
      color: f.group === "demand" ? colors.amber : f.is_streetview ? "#0e7c5a" : colors.jade,
    }));
    renderBarChart(featureSvg, items, { maxBars: 24 });
  }

  const proxySvg = document.getElementById("shapProxyChart");
  if (proxySvg && shap.proxyDecomposition && shap.proxyDecomposition.length) {
    const items = shap.proxyDecomposition.map((p) => ({
      label: `${p.label}  ${p.change_pct > 0 ? "+" : ""}${p.change_pct}%`,
      value: Math.abs(p.change_pct),
      display: `${p.change_pct > 0 ? "+" : ""}${p.change_pct}%`,
      color: p.change_pct > 0 ? colors.jade : colors.terracotta,
    }));
    renderBarChart(proxySvg, items);
  }

  const highlightsContainer = document.getElementById("shapHighlights");
  if (highlightsContainer) {
    clear(highlightsContainer);
    (shap.highlights || []).forEach((item) => {
      const card = create("article", "sample-card");
      card.append(
        create("h3", "", item.title),
        create("p", "", item.detail),
